class AcademicDetailsController < ApplicationController
  ...
  def update
    respond_to do |format|
      puts academic_detail_params
      puts params[:academic_detail][:pg_authority]
  ...
  def academic_detail_params
    params.require(:academic_detail).permit(:sslc_mark, :sslc_institute, :sslc_authority,:sslc_year,:hsc_mark, :hsc_institute, :hsc_authority,:hsc_year,:ug_mark, :ug_institute, :ug_authority,:ug_year, :pg_mark, :pg_institute, :pg_authority,:pg_year,:user_id)
  end
  def sign_in_check
